  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <select
          value={size}
          onChange={(e) => { setPage(1); setSize(parseInt(e.target.value)); }}
          className="h-11 rounded-2xl border px-3"
        >
          ...
          <tbody>
            {loading && (
              Array.from({ length: size }).map((_, i) => (
                <tr key={i} className="animate-pulse">
                  ...
                  <td className="px-4 py-3"><div className="h-4 rounded bg-gray-200"/></td>
                  <td className="px-4 py-3 text-right"><div className="h-4 rounded bg-gray-200"/></td>
                  <td className="px-4 py-3 text-right"><div className="h-4 rounded bg-gray-200"/></td>
                  <td className="px-4 py-3 text-right"><div className="h-4 rounded bg-gray-200"/></td>
                </tr>
              ))
            ...
            {!loading && data?.items.map((g) => (
              <tr key={g.app_id} className="odd:bg-white even:bg-gray-50">
                ...
                <td className="px-4 py-3 text-right tabular-nums">{g.current ?? "—"}</td>
                <td className="px-4 py-3 text-right tabular-nums">{g.peak24 ?? "—"}</td>
                <td className="px-4 py-3 text-right tabular-nums">{g.peak ?? "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
